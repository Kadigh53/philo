...
/*   Updated: 2023/06/07 16:55:23 by aaoutem-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vars	*init_args(int ac, char **av, t_vars *vars)
{
	...
	vars->start_time = ft_gettime_inms();
	if (ac == 6)
	...
	sem_unlink("/prin_lock");
	vars->print_lock = sem_open("/prin_lock", O_CREAT, 0600, 1);
	if (vars->forks_sem == SEM_FAILED || vars->print_lock == SEM_FAILED)
	{
		printf("error\n");
		exit (0);
	}
