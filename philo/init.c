...
/*   Updated: 2023/06/23 23:18:05 by kadigh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_vars_struct(t_data **data, int ac, char **av)
{
	...
	if (!(*data)->vars->print_lock)
		return (-1);
	// (*data)->vars->eat_lock = ft_calloc(1, sizeof(pthread_mutex_t));
	// if (!(*data)->vars->eat_lock)
	// 	return (-1);
	if (pthread_mutex_init((*data)->vars->print_lock, NULL))
		return (-1);
	// if (pthread_mutex_init((*data)->vars->eat_lock, NULL))
	// 	return (-1);
	return (1);
}
